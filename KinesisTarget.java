...
import com.streamsets.pipeline.stage.lib.kinesis.*;
...
import static com.streamsets.pipeline.stage.lib.kinesis.KinesisUtil.*;
    ...
    if (issues.isEmpty()) {
      ...
      try {
        ...
        if (conf.region == AwsRegion.OTHER) {
            //Boris:added modification
            region=getRegionFromVPC(conf.endpoint);
            producerConfig.setRegion(region);
